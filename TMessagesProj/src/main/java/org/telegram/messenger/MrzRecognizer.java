...
public class MrzRecognizer {
	...
	private static Result recognizeBarcode(Bitmap bitmap) {
		...
		try {
			java.util.List<Barcode> barcodes = Tasks.await(detector.process(InputImage.fromBitmap(bitmap, 0)));
			for (int i = 0; i < barcodes.size(); i++) {
				Barcode code = barcodes.get(i);
				Barcode.DriverLicense driverLicense = code.getDriverLicense();
				if (code.getValueType() == Barcode.TYPE_DRIVER_LICENSE && driverLicense != null) {
					Result res = new Result();
					res.type = "ID".equals(driverLicense.getDocumentType()) ? Result.TYPE_ID : Result.TYPE_DRIVER_LICENSE;
					String issuingCountry = driverLicense.getIssuingCountry();
					if ("USA".equals(issuingCountry)) {
						res.nationality = res.issuingCountry = "US";
					} else if ("CAN".equals(issuingCountry)) {
						res.nationality = res.issuingCountry = "CA";
					}
					res.firstName = capitalize(driverLicense.getFirstName());
					res.lastName = capitalize(driverLicense.getLastName());
					res.middleName = capitalize(driverLicense.getMiddleName());
					res.number = driverLicense.getLicenseNumber();
					if (driverLicense.getGender() != null) {
						switch (driverLicense.getGender()) {
							case "1":
					...
					try {
						...
						if (birthDate != null && birthDate.length() == 8) {
							res.birthYear = Integer.parseInt(birthDate.substring(yearOffset, yearOffset + 4));
							res.birthMonth = Integer.parseInt(birthDate.substring(monthOffset, monthOffset + 2));
							res.birthDay = Integer.parseInt(birthDate.substring(dayOffset, dayOffset + 2));
						}
						String expiryDate = driverLicense.getExpiryDate();
						if (expiryDate != null && expiryDate.length() == 8) {
							res.expiryYear = Integer.parseInt(expiryDate.substring(yearOffset, yearOffset + 4));
							res.expiryMonth = Integer.parseInt(expiryDate.substring(monthOffset, monthOffset + 2));
							res.expiryDay = Integer.parseInt(expiryDate.substring(dayOffset, dayOffset + 2));
						}
					} catch (NumberFormatException ignore) {
					}

					return res;
				} else if (code.getValueType() == Barcode.TYPE_TEXT && code.getFormat() == Barcode.FORMAT_PDF417) {
					String rawValue = code.getRawValue();
					if (rawValue != null && rawValue.matches("^[A-Za-z0-9=]+$")) {
						try {
